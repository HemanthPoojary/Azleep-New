...
const JournalList: React.FC<JournalListProps> = ({ onEntrySaved }) => {
  ...
  const { data: journals, isLoading: isLoadingJournals, error, refetch } = useQuery({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end mb-2">
        <Button variant="outline" size="sm" onClick={() => refetch()} disabled={isLoadingJournals}>
          {isLoadingJournals ? 'Refreshing...' : 'Refresh'}
        </Button>
      </div>
