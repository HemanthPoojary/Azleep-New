...
const SleepCastPage = () => {
  ...
  return <PageContainer className="relative overflow-hidden">
      ...
      <div className="mb-6 animate-fade-in">
        <div className="rounded-lg bg-white/10 backdrop-blur-sm p-3 flex items-center mx-[240px] my-[65px]">
      ...
      <div className="flex flex-col items-center justify-center h-[40vh] md:h-[50vh] animate-fade-in">
        <div className={`w-60 h-60 mb-8 md:w-72 md:h-72 ${isPlaying ? 'shadow-glow' : ''}`} style={{
          ...
          <AspectRatio ratio={1 / 1} className="relative">
            <div className={`absolute inset-0 rounded-full sleep-card flex flex-col items-center justify-center ${isPlaying ? 'pulse-ring' : ''}`}>
              <div className="text-center px-4 mb-4">
                ...
                {isPlaying && <div className="flex items-end justify-center h-12 gap-1 mb-2">
                    {[0, 1, 2, 3, 4].map(i => <div key={i} className="bg-[#FBBF24] w-2 rounded-full" style={{
              ...
              <Button variant="outline" size="icon" className="h-14 w-14 rounded-full bg-[#FBBF24] hover:bg-[#F59E0B] text-white border-none" onClick={handlePlayPause}>
        ...
        <div className="flex items-center w-full max-w-xs md:max-w-sm mb-8">
          ...
          <Slider defaultValue={[volume]} max={100} step={1} className="w-full" onValueChange={handleVolumeChange} />
        </div>
      </div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
          {sleepCasts.map(cast => <div key={cast.id} className="animate-fade-in" style={{
          animationDelay: `${sleepCasts.indexOf(cast) * 0.1}s`
        }}>
      ...
      {isMobile && <div className="mt-8">
          ...
          <Carousel className="w-full">
            <CarouselContent>
              {sleepCasts.slice(0, 3).map(cast => <CarouselItem key={cast.id} className="md:basis-1/2 lg:basis-1/3">
                  <SleepCastCard {...cast} onPlay={() => handleCastSelect(cast)} />
                </CarouselItem>)}
            </CarouselContent>
          </Carousel>
